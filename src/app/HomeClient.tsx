...
import type { TopItem } from "@/lib/top";
...
export function HomeClient({ initialTop }: { initialTop: TopItem[] }) {
  const [url, setUrl] = useState("");
  const [style, setStyle] = useState<StyleId>(DEFAULT_STYLE);
  const [job, setJob] = useState<JobResponse | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [top, setTop] = useState<TopItem[]>(initialTop);
  ...
  // Refresh the server-rendered leaderboard after an analysis completes.
